...
const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      try {
        // Fetch user stats
        const statsResponse = await fetch(`https://padak-backend.onrender.com/api/dashboard/stats`, {
          headers: {
        ...
        if (user.accountType !== 'admin') {
          const coursesResponse = await fetch(`https://padak-backend.onrender.com/api/courses/enrolled`, {
            headers: {
              'Authorization': `Bearer ${token}`
            }
          });
          ...
          const internshipsResponse = await fetch(`https://padak-backend.onrender.com/api/user/internship-applications`, {
            headers: {
              'Authorization': `Bearer ${token}`
            }
          });
          ...
          if (['professional', 'business', 'agency'].includes(user.accountType)) {
            const servicesResponse = await fetch(`https://padak-backend.onrender.com/api/users/service-requests`, {
              headers: {
                'Authorization': `Bearer ${token}`
              }
            });
          ...
          const calendarResponse = await fetch(`https://padak-backend.onrender.com/api/calendar/events`, {
            headers: {
              'Authorization': `Bearer ${token}`
            }
          });
