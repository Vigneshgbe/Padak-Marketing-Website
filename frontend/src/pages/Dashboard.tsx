    ...
    const fetchUserData = async () => {
      ...
      try {
        ...
        if (user.accountType !== 'admin') {
          const coursesResponse = await fetch(`${import.meta.env.VITE_API_URL}/api/courses/enrolled`, {
            headers: {
              'Authorization': `Bearer ${token}`
            }
          });
          ...
          const internshipsResponse = await fetch(`${import.meta.env.VITE_API_URL}/api/user/internship-applications`, {
            headers: {
              'Authorization': `Bearer ${token}`
            }
          });
          ...
          if (['professional', 'business', 'agency'].includes(user.accountType)) {
            const servicesResponse = await fetch(`${import.meta.env.VITE_API_URL}/api/users/service-requests`, {
              headers: {
                'Authorization': `Bearer ${token}`
              }
            });
          ...
          const calendarResponse = await fetch(`${import.meta.env.VITE_API_URL}/api/calendar/events`, {
            headers: {
              'Authorization': `Bearer ${token}`
            }
          });
