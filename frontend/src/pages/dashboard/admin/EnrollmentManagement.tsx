...
const EnrollmentManagement: React.FC = () => {
  ...
  const fetchRequests = async () => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/admin/enrollment-requests`, {
        method: 'GET',
  ...
  const fetchEnrollments = async () => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/admin/enrollments`, {
        method: 'GET',
        headers: getAuthHeaders(),
        credentials: 'include'
      });
  ...
  const handleApproveRequest = async (requestId: string) => {
    ...
    try {
      setSaving(true);
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/admin/enrollment-requests/${requestId}/approve`, {
        method: 'PUT',
        headers: getAuthHeaders(),
        credentials: 'include'
      });
  ...
  const handleRejectRequest = async (requestId: string) => {
    ...
    try {
      setSaving(true);
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/admin/enrollment-requests/${requestId}/reject`, {
        method: 'PUT',
        headers: getAuthHeaders(),
        credentials: 'include',
  ...
  const handleDeleteRequest = async (requestId: string) => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/admin/enrollment-requests/${requestId}`, {
        method: 'DELETE',
        headers: getAuthHeaders(),
        credentials: 'include'
      });
  ...
  const handleSaveEnrollment = async () => {
    ...
    try {
      setSaving(true);
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/admin/enrollments/${selectedEnrollment.id}`, {
        method: 'PUT',
        headers: getAuthHeaders(),
        credentials: 'include',
        body: JSON.stringify({
  ...
  const handleDeleteEnrollment = async (id: string) => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/admin/enrollments/${id}`, {
        method: 'DELETE',
        headers: getAuthHeaders(),
        credentials: 'include'
      });
              ...
              <div className="col-span-2">
                ...
                {selectedRequest.payment_screenshot ? (
                  <div className="relative group">
                    <img 
                      src={`${import.meta.env.VITE_API_URL}${selectedRequest.payment_screenshot}`} 
                      alt="Payment Proof"
                      ...
                      onClick={() => handleImageZoom(`${import.meta.env.VITE_API_URL}${selectedRequest.payment_screenshot}`)}
                      onLoad={(e) => {
                        console.log('✅ Image loaded successfully:', `${import.meta.env.VITE_API_URL}${selectedRequest.payment_screenshot}`);
                      }}
                      onError={(e) => {
                        console.error('❌ Image load error:', `${import.meta.env.VITE_API_URL}${selectedRequest.payment_screenshot}`);
