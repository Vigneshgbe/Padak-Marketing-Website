...
const PaymentManagement: React.FC = () => {
  ...
  const fetchPayments = async () => {
    try {
      const response = await fetch(`https://localhost:5000/api/admin/payments`, {
  ...
  const verifyPayment = async (paymentId: number, status: 'approved' | 'rejected') => {
    try {
      const response = await fetch(`https://localhost:5000/api/admin/payments/${paymentId}/verify`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
  ...
  const viewProof = (proofFile: string) => {
    window.open(`https://localhost:5000/uploads/payments/${proofFile}`, '_blank');
  };
