...
const SocialFeed: React.FC = () => {
  ...
  const fetchPosts = async (pageNum: number = 1) => {
    try {
      ...
      const response = await fetch(`https://padak-backend.onrender.com/api/posts?page=${pageNum}&limit=10`, {
        headers: {
  ...
  const handleCreatePost = async () => {
    ...
    try {
      const response = await fetch(`https://padak-backend.onrender.com/api/posts`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`
        },
  ...
  const handleEditPost = async (postId: number) => {
    ...
    try {
      const response = await fetch(`https://padak-backend.onrender.com/api/posts/${postId}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
  ...
  const handleEngagement = async (postId: number, type: string, content = '') => {
    try {
      const response = await fetch(`https://padak-backend.onrender.com/api/posts/${postId}/${type}`, {
  ...
  const handleShare = async (postId: number) => {
    try {
      ...
      // Track share
      await fetch(`https://padak-backend.onrender.com/api/posts/${postId}/share`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`
        }
