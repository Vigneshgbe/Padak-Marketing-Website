...
const SocialFeed: React.FC = () => {
  ...
  const fetchPosts = async (pageNum: number = 1) => {
    try {
      ...
      const response = await fetch(`https://localhost:5000/api/posts?page=${pageNum}&limit=10`, {
        headers: {
  ...
  const handleEngagement = async (postId: number, type: string, content = '') => {
    try {
      const response = await fetch(`https://localhost:5000/api/posts/${postId}/${type}`, {
  ...
  const handleShare = async (postId: number) => {
    try {
      ...
      // Track share
      await fetch(`https://localhost:5000/api/posts/${postId}/share`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`
        }
