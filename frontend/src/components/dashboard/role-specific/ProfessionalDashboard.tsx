...
const ProfessionalDashboard: React.FC<ProfessionalDashboardProps> = ({ user, stats, onViewChange }) => {
  ...
  useEffect(() => {
    const fetchEnrolledCourses = async () => {
      ...
      try {
        const response = await fetch(`https://padak-backend.onrender.com/api/users/${user.id}/enrollments`, {
  ...
  useEffect(() => {
    const fetchAppliedInternships = async () => {
      ...
      try {
        const response = await fetch(`https://padak-backend.onrender.com/api/users/${user.id}/internship-submissions`, {
          headers: { 'Authorization': `Bearer ${token}` }
        });
        if (!response.ok) throw new Error(`HTTP error! Status: ${response.status}`);
  ...
  useEffect(() => {
    const fetchAvailableServices = async () => {
      ...
      try {
        ...
        const response = await fetch(`https://padak-backend.onrender.com/api/services`, {
           headers: { 'Authorization': `Bearer ${token}` }
        });
        if (!response.ok) throw new Error(`HTTP error! Status: ${response.status}`);
  ...
  useEffect(() => {
    const fetchMyServiceRequests = async () => {
      ...
      try {
        const response = await fetch(`https://padak-backend.onrender.com/api/users/${user.id}/service-requests`, {
          headers: { 'Authorization': `Bearer ${token}` }
        });
        if (!response.ok) throw new Error(`HTTP error! Status: ${response.status}`);
  ...
  const handleRequestServiceSubmit = async (requestData: {
    ...
    try {
      ...
      const response = await fetch(`https://padak-backend.onrender.com/api/service-requests`, {
        method: 'POST',
        headers: {
